...
func _run_tests() -> void:
	...
	for level in range(1, LEVELS_TO_TEST + 1):
		var theme := LevelGenerator.theme_for_level(level, themes)
		algorithms_seen[str(theme.get("generation", {}).get("algorithm", "?"))] = true
		var tag := "L%d/%s" % [level, theme.get("id", "?")]
		var layout := LevelGenerator.generate(level, theme)

		_check(layout["ok"], "%s: generation validated (ratio %.3f, attempts %d)" % [tag, layout["reachable_ratio"], layout["attempts"]])
		_check(layout["reachable_ratio"] >= LevelGenerator.MIN_REACHABLE_RATIO, "%s: reachable ratio %.3f" % [tag, layout["reachable_ratio"]])
		...
		# Spawn circle is clear of prop footprints.
		...
		# Determinism: regenerate → identical layout.
		var layout2 := LevelGenerator.generate(level, theme)
	...
	# Mechanical variation: same theme, different levels → different layouts.
	var theme0 := LevelGenerator.theme_for_level(1, themes)
	var a := LevelGenerator.generate(1, theme0)
	var b := LevelGenerator.generate(1 + themes.size(), theme0)
